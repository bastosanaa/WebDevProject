...
export const updateUser = async (id: string, userData: { nome?: string; email?: string; senha?: string }) => {
    try {
        const response = await axios.patch(`${entityBaseUrl}/update/${id}`, userData);
        return response.data;
    } catch (error) {
        if (axios.isAxiosError(error)) {
