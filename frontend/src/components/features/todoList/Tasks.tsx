...
import {getTasksByUser, deleteTask, updateTaskStatus} from "../../../service/tasks";
                    ...
                    <div className="tasks-group scale-125 overflow-y-auto h-80 rounded-2xl m-0">
                        <ul>
                            {tasks.map((task, index) => (
                                <li key={index} className={`flex justify-between items-center ${task.em_andamento ? 'bg-rosa-claro':'border border-vermelho-encerrado'}`}>
                                    <button className="p-4" onClick={() => onTaskClick(task)}>
                                        {task.titulo}
                                    </button>
                                    <div className="flex space-x-2 p-2">
                                        <button className="end" title='Encerrar tarefa' onClick={() => handleEndTask(task._id)}><BsCheckLg /></button>
                                        <button className="edit" title='Editar tarefa'><BsPencilSquare /></button>
                                        <button className='delete' title='Excluir tarefa' onClick={() => handleDeleteTask(task._id)}><BsFillTrash3Fill /></button>
                                    </div>
                                </li>
                            ))}
                        </ul>
